...
export async function create({name, username, password, last_name, mail}: QueryCreate):Promise<User> { 
    const salt = await genSalt(saltRounds);
    const hashed = await hash(password, salt); 

    const res = await connection.query(sql`
        INSERT INTO users (username, name, last_name, password, mail)
        VALUES (${sql.join([username, name, last_name, hashed, mail], sql`, `)}) RETURNING id;
    `);
...
export async function findUserByUsernameWithPassword(username: string):Promise< UserWithPassword> { 
    const res = await connection.query(sql`SELECT * FROM users WHERE username = ${username}`);
    if (res.rows.length) {
...
function usererify(row: QueryResultRow): User {
    const { 
        id, last_name, mail, name, username, created_at, modified_at
    } = row;
    return { id, last_name, mail, name, username, created_at, modified_at } as any
}